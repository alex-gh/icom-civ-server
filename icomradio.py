...
class IcomRadio:
    ...
    def cmd(self, cmd_bytes, payload=b'', read_cmd=False):
    ...
    def read_agc(self):
        resp = self.cmd(b'\x16\x12', read_cmd=True)
        if resp == b'\x01':
            return 'Fast'
        elif resp == b'\x02':
            return 'Slow'
    ...
    def read_freq(self):
        resp = self.cmd([0x03], read_cmd=True)
        f = resp[8:10] + resp[6:8] + resp[4:6]
        f += resp[2:4] + resp[0:2]
    ...
    def read_mode(self):
        resp = self.cmd(b'\x04', read_cmd=True)
